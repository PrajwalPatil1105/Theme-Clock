...
setInterval(function () {
  ...
  seconds = currentDay.getSeconds() * 6;
  minute = currentDay.getMinutes() * 6;
  hour = currentDay.getHours() * 30 + Math.round(minute / 12);
  shand.style.transform = "rotate(" + seconds + "deg)";
  mhand.style.transform = "rotate(" + minute + "deg)";
  hhand.style.transform = "rotate(" + hour + "deg)";

  display.value = currentDay.toLocaleTimeString("en-US");
  let a = currentDay.getDay();
  let b = currentDay.getMonth();
  let c = currentDay.getDate();
